...
const struct fb_videomode tegra_dc_hdmi_supported_modes[] = {
	...
	/* 1280x720p 50hz: EIA/CEA-861-B Format 19 */
	{
		.xres =     1280,
		.yres =     720,
		.pixclock = KHZ2PICOS(74250),
		...
		.left_margin =  220,    /* h_back_porch */
		.upper_margin = 20, /* v_back_porch */
		.right_margin = 440,    /* h_front_porch */
		.lower_margin = 5,  /* v_front_porch */
		.vmode =    FB_VMODE_NONINTERLACED,
		.sync = FB_SYNC_HOR_HIGH_ACT | FB_SYNC_VERT_HIGH_ACT,
		.refresh = 50,
	...
	/* 1920x1080p 50hz EIA/CEA-861-B Format 31 */
	{
		.xres =     1920,
		.yres =     1080,
		.pixclock = KHZ2PICOS(148500),
		...
		.left_margin =  148,    /* h_back_porch */
		.upper_margin = 36, /* v_back_porch */
		.right_margin = 528,/* h_front_porch */
		.lower_margin = 4,  /* v_front_porch */
		.vmode =    FB_VMODE_NONINTERLACED,
		.sync = FB_SYNC_HOR_HIGH_ACT | FB_SYNC_VERT_HIGH_ACT,
		.refresh = 50,
...
const struct fb_videomode *hdmi_find_best_mode(const struct fb_var_screeninfo *var)
{
	int i = 0, var_h_blanking, hdmi_h_blanking;
	int len = sizeof(tegra_dc_hdmi_supported_modes) / sizeof(struct fb_videomode);
	const struct fb_videomode *best = NULL;

	var_h_blanking = var->hsync_len + var->left_margin + var->right_margin;
	for (i = 0; i < len; i++) {
		if (tegra_dc_hdmi_supported_modes[i].xres == var->xres && tegra_dc_hdmi_supported_modes[i].yres == var->yres) {
			hdmi_h_blanking = tegra_dc_hdmi_supported_modes[i].hsync_len + tegra_dc_hdmi_supported_modes[i].left_margin + tegra_dc_hdmi_supported_modes[i].right_margin;
			if (var_h_blanking == hdmi_h_blanking) {
				best = &(tegra_dc_hdmi_supported_modes[i]);
			}
		}
	}
...
static void tegra_dc_hdmi_early_suspend(struct early_suspend *h)
{
	struct tegra_dc_hdmi_data *hdmi = container_of(h, struct tegra_dc_hdmi_data, early_suspend);
	unsigned long flags;
...
static void tegra_dc_hdmi_resume_worker(struct work_struct *work)
{
	struct tegra_dc_hdmi_data *hdmi = container_of(work, struct tegra_dc_hdmi_data, resume_work);
	unsigned long flags;
...
static void tegra_dc_hdmi_late_resume(struct early_suspend *h)
{
	struct tegra_dc_hdmi_data *hdmi = container_of(h, struct tegra_dc_hdmi_data, early_suspend);
	queue_work(system_nrt_wq, &hdmi->resume_work);
}
#endif
...
static int tegra_dc_hdmi_init(struct tegra_dc *dc)
{
	...
	hdmi->early_suspend.suspend = tegra_dc_hdmi_early_suspend;
	hdmi->early_suspend.resume = tegra_dc_hdmi_late_resume;
	register_early_suspend(&hdmi->early_suspend);
	INIT_WORK(&hdmi->resume_work, tegra_dc_hdmi_resume_worker);
#endif
